...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Adriano Marto Reis, modified for SHPI");
...
static void soft_uart_set_termios(struct tty_struct* tty, struct ktermios* termios)
{
  ...
  if (cflag & CSTOPB)
  {
    raspberry_soft_uart_set_two_stops(1);
    printk(KERN_INFO "soft_uart: 2 stop bits.\n");
  } else {
  raspberry_soft_uart_set_two_stops(0);
  }
  
  // Verifies the parity (it must be none).
  if (cflag & PARENB)
  {
    raspberry_soft_uart_set_parity(1);
    printk(KERN_INFO "soft_uart: Parity added.\n");
  } else {raspberry_soft_uart_set_parity(0);}
